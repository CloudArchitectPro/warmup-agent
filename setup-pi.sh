...
# ─── Verify running on Pi as non-root ─────────────────────────────────────────
[[ "$(id -u)" != "0" ]] || die "Do not run this script as root. Run as the 'pi' user."

AGENT_DIR="/home/pi/warmup-agent"
...
# ─── Fix any /home/ubuntu/ references → /home/pi/ ────────────────────────────
info "Normalising paths from /home/ubuntu/ to /home/pi/ ..."
for f in ecosystem.config.js .env.example; do
  if [ -f "$f" ]; then
    sed -i 's|/home/ubuntu/|/home/pi/|g' "$f"
  fi
done
success "Paths normalised"
...
# ─── SQLite DB path ───────────────────────────────────────────────────────────
DB_PATH=/home/pi/warmup-agent/data/warmup.db

# ─── IMAP passwords (fill in each one) ───────────────────────────────────────
IMAP_PASSWORD_CLOUMA_COM=
IMAP_PASSWORD_NUVATRON_COM=
IMAP_PASSWORD_MEDICALBROTHERS_COM=
IMAP_PASSWORD_NAVEEN_CLOUD=
IMAP_PASSWORD_XAIPEX_COM=
IMAP_PASSWORD_SANTHIGIRI_ORG=
IMAP_PASSWORD_THARUNMOORTHY_COM=
IMAP_PASSWORD_NIMBUSNEBULA_COM=
IMAP_PASSWORD_FLIPPYFLY_COM=
IMAP_PASSWORD_INDXPRO_COM=
IMAP_PASSWORD_EXAMTRAPS_COM=

# ─── Optional IMAP user override (blank = use first sender) ──────────────────
# IMAP_USER_CLOUMA_COM=
# IMAP_USER_NUVATRON_COM=
EOF
...
if [ -n "$PM2_STARTUP" ]; then
  ...
  pm2 startup systemd -u pi --hp /home/pi || warn "Could not auto-configure PM2 startup — run 'pm2 startup' manually"
fi
